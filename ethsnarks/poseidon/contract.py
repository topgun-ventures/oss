...
def _add_round_key(r, t, K):
    ...
    """
    middle = [
        [DUP(1 + t), DUP(1), DUP(3 + i), ADDMOD, SWAP(2 + i), POP] for i in range(t)
    ]
...
def poseidon_contract_opcodes(params=None):
    ...
    yield [
        [PUSH(M[i][j]), PUSH((1 + i * params.t + j) * 32), MSTORE]
    ...
    """
    yield [[PUSH(0x44 + (0x20 * (5 - i))), CALLDATALOAD] for i in range(params.t)]
    ...
    for i in range(params.nRoundsF + params.nRoundsP):
        ...
        if i < (params.nRoundsF // 2) or i >= (
            params.nRoundsP + (params.nRoundsF // 2)
        ):
            for j in range(params.t):
        ...
        yield [PUSHLABEL(label), PUSH(0), MSTORE, JMP("mix"), LABEL(label)]
    ...
    mix();
    """
    yield [PUSH(0), MSTORE, PUSH(0x20), PUSH(0), RETURN]  # Save it to pos 0
