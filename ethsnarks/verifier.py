...
_VerifyingKeyStruct = namedtuple(
    "_VerifyingKeyStruct", ("alpha", "beta", "gamma", "delta", "gammaABC")
)

_ProofStruct = namedtuple("_ProofStruct", ("A", "B", "C", "input"))
...
def _bigint_bytes_to_int(x):
    ...
    return reduce(
        lambda o, b: (o << 8) + b if isinstance(b, int) else ord(b), [0] + list(x)
    )
...
def _load_g2_point(point):
    ...
    out = (
        FQ2([_filter_int(x[1]), _filter_int(x[0])]),
