...
def mimc_contract_opcodes(exponent):
    ...
    yield [LABEL("start"), PUSH(SNARK_SCALAR_FIELD), PUSH(0x24), MLOAD]  # q  # k q

    yield [PUSH(0x04), MLOAD]  # 0x04 k q  # x k q
    ...
    return [
        {
            "constant": True,
            "inputs": [
                {"name": "in_x", "type": "uint256"},
                {"name": "in_k", "type": "uint256"},
            ],
            "name": f"MiMCpe{exponent}",
            "outputs": [{"name": "out_x", "type": "uint256"}],
            "payable": False,
