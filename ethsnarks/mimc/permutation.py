...
def mimc(
    x, k, seed=DEFAULT_SEED, p=SNARK_SCALAR_FIELD, e=DEFAULT_EXPONENT, R=DEFAULT_ROUNDS
):
    """
...
def _main():
    ...
    parser.add_argument(
        ...
        help="exponent for round function",
    )
    parser.add_argument(
        "-s",
        "--seed",
        ...
        help="seed for round constants",
    )
    parser.add_argument("-k", "--key", type=int, default=0, help="initial key")
    parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, help="display settings"
    )
    ...
    if cmd == "test":
        ...
        assert (
            mimc(1, 1)
            == 2447343676970420247355835473667983267115132689045447905848734383579598297563
        )
        assert (
            mimc_hash([1, 1])
            == 4087330248547221366577133490880315793780387749595119806283278576811074525767
        )
        ...
        assert (
            ...
            == 11437467823393790387399137249441941313717686441929791910070352316474327319704
        )
        assert (
            mimc_hash(
                [
